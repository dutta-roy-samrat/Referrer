...
const ChipsInput = forwardRef<HTMLInputElement, { chips?: string[]; setChips: Dispatch<SetStateAction<string[]>> }>(
  ({ chips = defaultArr, setChips }, ref) => {
    ...
    const handleInputChange: ChangeEventHandler<HTMLInputElement> = (event) => {
      ...
      if (value.includes(",")) {
        ...
        setChips((prevChips) => [...prevChips, ...trimmedChips].slice(0, maxChips));
    ...
    return (
      <>
        ...
        <p className={`mt-2 text-sm ${chips.length >= maxChips ? "text-red-500" : "text-gray-600"}`}>
          {chips.length >= maxChips ? "Maximum chips reached. Remove a chip to add more." : `${chips.length}/${maxChips} skills added.`}
        </p>
      </>
    );
  }
